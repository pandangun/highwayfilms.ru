    ...
    <>
      <header className="page-head">
        <div className="wrap">
          ...
          />
          <p className="mt-10 max-w-[44em] text-steel">{getLocalizedText(p.summary, locale)}</p>
        </div>
      </header>
          ...
          <ol className="doc-rows">
            {p.versions.map((version) => (
              <li key={version.name} className="doc-row">
                ...
                <p className="text-steel">{getLocalizedText(version.note, locale)}</p>
                <div className="doc-row__meta">
                  <span>{getLocalizedText(version.state, locale)}</span>
                  <span className="text-haze">{formatProjectDate(version.updatedAt, locale)}</span>
                </div>
              </li>
            ))}
          </ol>
          ...
          <ul className="doc-rows">
            {p.comments.map((comment) => (
              <li key={`${comment.timecode}-${comment.author}`} className="doc-row">
                <p className="display display--h4 num text-led">{comment.timecode}</p>
                <p>{getLocalizedText(comment.text, locale)}</p>
                <div className="doc-row__meta">
                  <span>{getLocalizedText(comment.state, locale)}</span>
                  <span className="text-haze">{comment.author}</span>
                </div>
              </li>
            ))}
          </ul>
      ...
      <section className="band border-t border-line">
        <div className="wrap">
          ...
          <ul className="doc-rows">
            {p.assets.map((asset) => (
              <li key={asset.name} className="doc-row">
                ...
                <p className="text-steel">{getLocalizedText(asset.note, locale)}</p>
                <div className="doc-row__meta">
                  <span className="num">
          ...
          <p className="mt-6 text-micro text-haze">{t.demo}</p>
        </div>
      </section>
      ...
      <section className="invite">
        <div className="wrap">
          ...
          <div className="invite__actions">
            <a href={contacts.telegramHref} target="_blank" rel="noopener noreferrer" className="btn btn--primary">
              {t.write}
            </a>
          ...
          <p className="mt-12 text-small text-steel">{getLocalizedText(p.accessNote, locale)}</p>
        </div>
      </section>
    </>
  );

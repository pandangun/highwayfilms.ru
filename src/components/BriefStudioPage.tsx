...
function Field({
  ...
  return (
    <div className="brief-field">
      <div className="brief-field__head">
        <label htmlFor={htmlFor} className="brief-field__label">
          {label}
        </label>
        <span className="brief-field__hint">{hint}</span>
      </div>
      <div className="brief-field__body">{children}</div>
    </div>
  );
}
...
function Section({
  ...
  return (
    <section id={`brief-${step}`} className="brief-section-panel">
...
export function BriefStudioPage({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="page-shell brief-page-shell">
            ...
            <form id="contact-form" action="/api/contact" method="POST" className="brief-form mx-auto mt-8 w-full max-w-[980px] space-y-10">
              ...
              <Section step="project" badge="1/5" title={steps[0].title} description={steps[0].description}>
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Бренд / компания / продукт", "Brand / company / product")} htmlFor="company" hint={t("Необязательно", "Optional")}>
                    <input id="company" name="company" placeholder={t("Название бренда или продукта", "Brand or product name")} className="brief-form-control brief-form-control--input" />
                  </Field>
                  <Field label={t("Тип проекта", "Project type")} htmlFor="projectType" hint={t("Необязательно", "Optional")}>
                    <select id="projectType" name="projectType" defaultValue="" className="brief-form-control brief-form-control--input brief-form-control--select">
                ...
                <Field label={t("Что за задача", "What is the task")} htmlFor="background" hint={t("Желательно", "Recommended")}>
                  <textarea id="background" name="background" rows={6} placeholder={t("Что нужно снять, для какого запуска и почему проект появился сейчас.", "What needs to be produced, for which launch, and why the project exists now.")} className="brief-form-control brief-form-control--textarea" />
                </Field>
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Бизнес-цель", "Business goal")} htmlFor="businessGoals" hint={t("Необязательно", "Optional")}>
                    <textarea id="businessGoals" name="businessGoals" rows={5} placeholder={t("Продажи, знание, запуск, найм или другая конкретная цель.", "Sales, awareness, launch, hiring, or another concrete goal.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                  <Field label={t("Бюджетная рамка", "Budget frame")} htmlFor="budget" hint={t("Необязательно", "Optional")}>
                    <select id="budget" name="budget" defaultValue="" className="brief-form-control brief-form-control--input brief-form-control--select">
                ...
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Дата запуска / дедлайн", "Launch date / deadline")} htmlFor="targetDate" hint={t("Необязательно", "Optional")}>
                    <input id="targetDate" name="targetDate" type="date" className="brief-form-control brief-form-control--input" />
                  </Field>
                  <Field label={t("География / место съёмки", "Geography / shoot location")} htmlFor="location" hint={t("Необязательно", "Optional")}>
                    <input id="location" name="location" placeholder={t("Москва / несколько городов / travel shoot", "Moscow / multiple cities / travel shoot")} className="brief-form-control brief-form-control--input" />
                  </Field>
                </div>
              </Section>
              ...
              <Section step="audience" badge="2/5" title={steps[1].title} description={steps[1].description}>
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Для кого это видео", "Who this video is for")} htmlFor="audience" hint={t("Желательно", "Recommended")}>
                    <textarea id="audience" name="audience" rows={6} placeholder={t("Кто смотрит ролик, кто решает и что важно для этой аудитории.", "Who watches the video, who decides, and what matters to this audience.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                  <Field label={t("Коммуникационная цель", "Communication goal")} htmlFor="communicationGoals" hint={t("Необязательно", "Optional")}>
                    <textarea id="communicationGoals" name="communicationGoals" rows={6} placeholder={t("Какое действие, ощущение или сдвиг в восприятии должен дать ролик.", "What action, feeling, or perception shift the video should create.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                </div>
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Рынок / конкуренты", "Market / competitors")} htmlFor="marketSituation" hint={t("Необязательно", "Optional")}>
                    <textarea id="marketSituation" name="marketSituation" rows={6} placeholder={t("Кто уже говорит в категории и на каком поле мы оказываемся.", "Who already speaks in this category and what field we are entering.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                  <Field label={t("Как хотим отличаться", "How we want to stand out")} htmlFor="differentiation" hint={t("Необязательно", "Optional")}>
                    <textarea id="differentiation" name="differentiation" rows={6} placeholder={t("Что должно выделить проект: тон, аргумент, визуальный код или темп.", "What should differentiate the project: tone, argument, visual code, or pace.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                </div>
              </Section>
              ...
              <Section step="creative" badge="3/5" title={steps[2].title} description={steps[2].description}>
                <Field label={t("Главное сообщение", "Core message")} htmlFor="keyMessage" hint={t("Желательно", "Recommended")}>
                  <textarea id="keyMessage" name="keyMessage" rows={6} placeholder={t("Что зритель должен унести после просмотра.", "What the audience should carry away after watching.")} className="brief-form-control brief-form-control--textarea" />
                </Field>
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Стиль / ограничения", "Style / restrictions")} htmlFor="brandStyle" hint={t("Необязательно", "Optional")}>
                    <textarea id="brandStyle" name="brandStyle" rows={6} placeholder={t("Брендбук, визуальные пожелания, no-go зоны и ограничения.", "Brand book, visual wishes, no-go zones, and restrictions.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                  <Field label={t("Референсы и ссылки", "References and links")} htmlFor="references" hint={t("Необязательно", "Optional")}>
                    <textarea id="references" name="references" rows={6} placeholder={t("Ролики, moodboards, deck, сценарии или любые полезные материалы.", "Videos, moodboards, decks, scripts, or any useful materials.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                </div>
              </Section>
              ...
              <Section step="production" badge="4/5" title={steps[3].title} description={steps[3].description}>
                ...
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Нужный результат", "Expected output")} htmlFor="resultExpectation" hint={t("Необязательно", "Optional")}>
                    <textarea id="resultExpectation" name="resultExpectation" rows={6} placeholder={t("Например: treatment, смета, мастер-ролик или комплект версий.", "For example: treatment, estimate, master video, or a package of versions.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                  <Field label={t("Хронометраж и версии", "Runtime and versions")} htmlFor="runtimeVersions" hint={t("Необязательно", "Optional")}>
                    <textarea id="runtimeVersions" name="runtimeVersions" rows={6} placeholder={t("Например: 15 сек master, 10 сек cutdown, вертикаль 9:16.", "For example: 15 sec master, 10 sec cutdown, 9:16 vertical.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                </div>
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Технические требования", "Technical requirements")} htmlFor="technicalRequirements" hint={t("Необязательно", "Optional")}>
                    <textarea id="technicalRequirements" name="technicalRequirements" rows={6} placeholder={t("Safe zones, legal, CTA, титры, ограничения площадок.", "Safe zones, legal, CTA, captions, and platform limits.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                  <Field label={t("Что уже есть на руках", "What already exists")} htmlFor="assets" hint={t("Необязательно", "Optional")}>
                    <textarea id="assets" name="assets" rows={6} placeholder={t("Исходники, брендбук, продукт, музыка, сценарий или другие материалы.", "Footage, brand book, product assets, music, script, or other materials.")} className="brief-form-control brief-form-control--textarea" />
                  </Field>
                </div>
              </Section>
              ...
              <Section step="contact" badge="5/5" title={steps[4].title} description={steps[4].description}>
                ...
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Имя", "Name")} htmlFor="name" hint={t("Необязательно", "Optional")}>
                    <input id="name" name="name" placeholder={t("Как к вам обращаться", "How should we address you?")} className="brief-form-control brief-form-control--input" />
                  </Field>
                  <Field label="E-mail" htmlFor="email" hint={t("Одно из двух", "One of two")}>
                    <input id="email" name="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} required={phone.trim().length === 0} aria-describedby="contact-note" placeholder="you@example.com" className="brief-form-control brief-form-control--input" />
                  </Field>
                </div>
                <div className="grid gap-5 xl:grid-cols-2">
                  <Field label={t("Телефон / Telegram", "Phone / Telegram")} htmlFor="phone" hint={t("Одно из двух", "One of two")}>
                    <input id="phone" name="phone" value={phone} onChange={(event) => setPhone(event.target.value)} required={email.trim().length === 0} aria-describedby="contact-note" placeholder={t("+7 999 123-45-67 / @telegram", "+7 999 123-45-67 / @telegram")} className="brief-form-control brief-form-control--input" />
                  </Field>

...
export type WeddingQuote = {
  /** Номинация. Английская — она и есть сигнал «это язык кинофестиваля». */
  award: string;
  text: string;
  /**
   * Кто это сказал: «Аня и Миша». Пока пусто — подпись просто не выводится.
   * Заполнить стоит: отзыв с именем убеждает в разы сильнее анонимного,
   * а вся конструкция с номинациями перестаёт быть просто приёмом.
   */
  author?: string;
  /** Месяц и год свадьбы: «июль 2025». Выводится рядом с именем. */
  date?: string;
};
...
/**
 * Отзывы пар, разложенные по номинациям кинофестиваля. Это приём, а не
 * регалии, и на странице так и написано прямым текстом — иначе «Grand Prix»
 * без указания фестиваля читается как настоящая награда.
 *
 * Поля author и date пустые: подставить реальные имена и даты. Пока пусто,
 * подпись не выводится — ничего выдуманного на странице не появится.
 */
